...
class ChatChain:
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                self.chat_env = self.phases[phase].execute(self.chat_env,
                                                           ...
                                                           need_reflect)
                if self.chat_env.codes.codebooks:
                    return True
            else:
    ...
    def execute_chain(self):
        ...
        for phase_item in self.chain:
            if self.execute_step(phase_item):
                break
    ...
    def post_processing(self):
        ...
        # 尝试移动日志文件到 WareHouse 子目录，若不存在则跳过
        try:
            ...
            print(f"Log file not found, skip moving: {self.log_filepath}")
...
then you should return a message in a format like \"<INFO> revised_version_of_the_description\", do not return messages in other formats.""".format(
    ...
    def run_all_and_get_codebooks(self):
        """
        执行完整流程并返回生成的代码字典
        """
        self.pre_processing()
        ...
        print(self.chat_env.codes.codebooks)
        return self.chat_env.codes.codebooks
